...
export function Layout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {/* Sidebar */}
      <div 
        className={`fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out md:static md:translate-x-0 \${
          ...
          {user ? (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 overflow-hidden">
                <img src={user.photoURL || `https://ui-avatars.com/api/?name=${user.displayName}`} alt="Profile" className="w-8 h-8 rounded-full" />
                <span className="text-sm font-medium text-gray-700 truncate">{user.displayName}</span>
              </div>
              <button onClick={logOut} className="p-2 text-gray-500 hover:text-red-600 rounded-md hover:bg-gray-100" title="Cerrar sesión">
                <LogOut size={18} />
              </button>
            </div>
          ) : (
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <header className="flex items-center justify-between h-16 px-4 bg-white border-b border-gray-200 md:hidden">
          ...
          <span className="text-lg font-semibold text-gray-800">Gilda Study v2</span>
